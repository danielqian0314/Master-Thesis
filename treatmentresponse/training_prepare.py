...
def get_data_central_CT_2D(patientIDs):
    patientsCT=np.load(data_path+'patientsCT.npy').astype(int)
    CT2d=np.load(data_path+'CT2d.npy')
    PETCT2d=np.load(data_path+'PETCT2d.npy')
    input_CT2d=[]
    input_PETCT2d=[]
    for k in range(len(patientIDs)):        
        for i in range(len(patientsCT)):            
            if patientsCT[i]==patientIDs[k]:
                 where = np.array(np.where(CT2d[i*64:(i+1)*64]))
                 x2, y2, z2 = np.amax(where, axis=1)
                central_num=int(x2//2)
                input_CT2d.extend(MR2d[i*64+central_num-1:i*64+central_num+1])
                input_PETCT2d.extend(PETCT2d[i*64+central_num-1:i*64+central_num+1])
                break
